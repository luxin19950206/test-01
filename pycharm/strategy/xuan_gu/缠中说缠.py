...
class CALCINFO(object):
    ...
    # 上面的self其实就是下面的def再用到self时可以直接调用
    def _initStockData(self, df):  # 股票数据
        stockData = []  # 创建序列
...
class ChanlunCore:
    ...
    def initBiQK(self, pData):  # 初始化缺口数据
        qk = 0.005
        if pData.m_nNumData == 0:
            return

        if pData.m_dataType == MIN1_DATA:  # 分钟数据
            self.biQuekou = qk  # 笔缺口
    ...
    def initFX(self):  # k线分型
        kxnum = len(self.kxData)  # k线数量
        ...
        while i < len(self.kxData) - 1:
            ...
            if not tjg1:
                ...
                # 标出顶底分型
            if tjg1 or tjd1:  # 分型顶、分型底
                if 0 == gdnum:
                    # 第一个分型
                    if tjg1:
                        kx.fxqj = self.kxData[i - 1].low  # 分型区间
                        kx.flag = DIR_UP  # 分型标记
                        kxlg = kx
                    elif tjd1:
