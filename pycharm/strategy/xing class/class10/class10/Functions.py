...
def save_stock_data_from_sina_to_h5(code_list, save_path=config.output_data_path + '/all_stock_data_h5.h5'):
    """
    ...
    for line in content.split('\n'):
        ...
        output_df.loc[now_time, '股票代码'] = code
        output_df.loc[now_time, '股票名称'] = line_split[0].split('="')[-1]
        output_df.loc[now_time, '开盘价'] = open_price
        output_df.loc[now_time, '最高价'] = float(line_split[4])
        output_df.loc[now_time, '最低价'] = float(line_split[5])
        output_df.loc[now_time, '最新价'] = float(line_split[3])
        output_df.loc[now_time, '昨收'] = float(line_split[2])
        output_df.loc[now_time, '时间'] = pd.to_datetime(line_split[-3] + u' ' + line_split[-2])

        # 存储数据至h5文件
        # 可以把hdf理解成一个数据库
        output_df.to_hdf(save_path, code, mode='a', append=True, format='table')
...
def auto_send_email(to_address, subject, content, from_address='your_email_address@foxmail.com'):
    ...
    while True:
        try:
            ...
            password = 'aaaabbbbccccdddd'  # 此为假密码
